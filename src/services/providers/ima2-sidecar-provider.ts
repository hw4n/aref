...
import {
  cancelIma2SidecarGeneration,
  pollIma2SidecarGeneration,
  startIma2SidecarGeneration,
  type Ima2SidecarReferenceImagePayload,
...
async function readReferenceImagePayload(
  ...
  index: number,
): Promise<Ima2SidecarReferenceImagePayload> {
  ...
  if (!mimeType || !SIDE_CAR_SUPPORTED_REFERENCE_TYPES.has(mimeType)) {
    throw new Error(
      `ChatGPT OAuth reference images must be PNG, JPG, or WEBP. ${asset.sourceName ?? asset.id} is not supported.`,
    );
  }
...
export const ima2SidecarGenerationProvider: OneShotGenerationProviderAdapter = {
  ...
  async generateImages(invocation, options): Promise<GenerationProviderResult> {
    ...
    const handleAbort = () => {
      void cancelIma2SidecarGeneration(submission.operationId);
    };
    ...
    try {
      while (true) {
        ...
        const snapshot = await pollIma2SidecarGeneration(submission.operationId);
        ...
        if (snapshot.status === "failed") {
          throw new Error(snapshot.error ?? "ChatGPT OAuth generation failed.");
        }
        ...
        await waitWithAbort(IMA2_SIDECAR_POLL_INTERVAL_MS, options.signal);
      }
    } finally {
